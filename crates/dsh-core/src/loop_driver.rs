...
async fn run_turn(agent: &Arc<Agent>, claimed: Vec<Message>) -> Result<(), crate::AgentError> {
    ...
    let decision = agent
        ...
        .await
        .map_err(|err| crate::AgentError::Loop(err.to_string()))?;
    ...
    loop {
        ...
        if matches!(assembler.finish(), dsh_llm::FinishReason::MaxTokens) {
            session.append(SessionEventData::StepEnd { turn, step });
            session.append(SessionEventData::TurnEnd {
                turn,
...
async fn resolve_config(agent: &Arc<Agent>, turn: u64) -> Result<LlmCallConfig, crate::AgentError> {
    ...
    let result = agent
        ...
        .await
        .map_err(|err| crate::AgentError::Loop(err.to_string()))?;
    serde_json::from_value(result).map_err(|err| crate::AgentError::Loop(err.to_string()))
}
