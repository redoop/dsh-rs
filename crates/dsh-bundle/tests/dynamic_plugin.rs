...
fn build_plugin() {
    let status = Command::new(env!("CARGO"))
        .args(["build", "-p", "dsh-plugin-hello"])
        .status()
        .expect("run cargo build");
    assert!(status.success(), "cargo build -p dsh-plugin-hello failed");
    assert!(
    ...
    runtime
        .register_adapter(
            ...
            Arc::new(dsh_llm::adapters::mock::MockAdapter::scripted(vec![
                ...
                dsh_llm::adapters::mock::MockAdapter::text_response("plugin said hello"),
            ])),
        )
        .unwrap();
