...
use crate::session::event::{now_ms, SessionEvent, SessionEventData, SessionId, TurnEndReason};
use crate::session::store::SESSIONS_SERVICE;
...
pub fn jsonl_persistence_plugin(dir: PathBuf) -> Arc<dyn Plugin> {
    plugin_with(
        ...
        move |ctx, _config: Value| {
            ...
            async move {
                let store = ctx
                    .get::<crate::api::services::SessionService>(SESSIONS_SERVICE)
                    .ok_or_else(|| cordis::Error::msg("sessions service missing"))?;
                let backend = Arc::new(JsonlPersistence::new(dir))
                    as Arc<dyn crate::api::services::SessionPersistenceApi>;
...
impl crate::api::services::SessionPersistenceApi for JsonlPersistence {
    fn on_event(&self, session: &str, event: &crate::types::SessionEvent) {
    ...
    fn load(&self, session: &str) -> std::io::Result<Vec<crate::types::SessionEvent>> {
