...
use crate::session::event::{now_ms, SessionEvent, SessionEventData, SessionHeader, SessionId};
...
impl SessionStore {
    ...
    pub fn create(&self, options: CreateSessionOptions) -> Arc<Session> {
        ...
        crate::api::events::emit(
            &self.inner.ctx,
            &crate::api::events::SessionCreatedPayload { session: id.clone() },
        );
        session
    }
    ...
    pub fn enter(&self, session: Arc<Session>) -> Result<(), crate::session::SessionError> {
        let mut map = self.inner.sessions.lock().unwrap();
        if map.contains_key(&session.id) {
            return Err(crate::session::SessionError::Duplicate(session.id.clone()));
        }
    ...
    pub fn announce(&self, session: &Arc<Session>) {
        crate::api::events::emit(
            &self.inner.ctx,
            &crate::api::events::SessionCreatedPayload {
    ...
    fn make_notifier(&self, session_id: SessionId) -> Arc<dyn Fn(&SessionEvent) + Send + Sync> {
        ...
        Arc::new(move |event: &SessionEvent| {
            ...
            crate::api::events::emit(
                &inner.ctx,
                &crate::api::events::SessionEventPayload {
    ...
    pub fn remove(&self, id: &str) -> Option<Arc<Session>> {
        ...
        if session.is_some() {
            crate::api::events::emit(
                &self.inner.ctx,
                &crate::api::events::SessionDisposedPayload {
    ...
    pub async fn flush(&self, session: &Arc<Session>) -> Result<(), crate::session::SessionError> {
        let backends = self.inner.backends.lock().unwrap().clone();
        for backend in &backends {
            backend.flush(&session.id).map_err(crate::session::SessionError::Io)?;
        }
        ...
        match result {
            Ok(_) => Ok(()),
            Err(err) => Err(crate::session::SessionError::Other(err.to_string())),
        }
    }
    ...
    pub fn fork(
    ...
    ) -> Result<Arc<Session>, crate::session::SessionError> {
        ...
        if let Some(turn) = open {
            return Err(crate::session::SessionError::OpenTurn(turn));
        }
...
pub fn session_plugin() -> Arc<dyn Plugin> {
    plugin("sessions", |ctx, _config: Value| async move {
        ...
        let api: Arc<dyn crate::api::services::SessionStoreApi> = Arc::new(store.clone());
        ctx.provide(
            SESSIONS_SERVICE,
            crate::api::services::SessionService::new(api),
        )
        .await?;
        Ok(())
    })
...
impl crate::api::services::SessionStoreApi for SessionStore {
    fn create(&self, options: CreateSessionOptions) -> Arc<dyn crate::api::services::SessionView> {
        let session = self.create(options);
        session as Arc<dyn crate::api::services::SessionView>
    }

    fn get(&self, id: &str) -> Option<Arc<dyn crate::api::services::SessionView>> {
        self.get(id).map(|s| s as Arc<dyn crate::api::services::SessionView>)
    }

    fn list(&self) -> Vec<Arc<dyn crate::api::services::SessionView>> {
        self.list()
            .into_iter()
            .map(|s| s as Arc<dyn crate::api::services::SessionView>)
            .collect()
    }

    fn remove(&self, id: &str) -> Option<Arc<dyn crate::api::services::SessionView>> {
        self.remove(id)
            .map(|s| s as Arc<dyn crate::api::services::SessionView>)
    }
    ...
    ) -> Result<Arc<dyn crate::api::services::SessionView>, String> {
        let source = self
            .inner
            .sessions
            .lock()
    ...
    fn flush(&self, session_id: &str) -> crate::api::services::BoxFuture<Result<(), String>> {
    ...
    fn attach_persistence(
        &self,
        backend: Arc<dyn crate::api::services::SessionPersistenceApi>,
    ) {
        self.inner
            .backends
            .lock()
...
struct BackendAdapter(Arc<dyn crate::api::services::SessionPersistenceApi>);

impl crate::session::persistence::SessionPersistence for BackendAdapter {
