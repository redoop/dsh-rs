...
use crate::api::services::AgentView;
use crate::types::{ContentBlock, SessionEvent, SessionEventData, TurnEndReason};
...
fn tool_result_text(message: &crate::types::Message) -> String {
...
fn message_is_error(message: &crate::types::Message) -> bool {
    ...
    pub fn apply_event(&mut self, event: &SessionEvent) -> bool {
        match &event.data {
            ...
            SessionEventData::AssistantChunk { chunk, .. } => {
                if let crate::types::StreamChunk::TextDelta { text, .. } = chunk {
...
pub async fn attach_listener(
    ...
    crate::api::events::on::<crate::api::events::SessionEventPayload, _>(ctx, move |_ctx, payload| {
    ...
    pub fn handle_key(&mut self, key: ratatui::crossterm::event::KeyEvent, agent: &Arc<dyn AgentView>) -> io::Result<bool> {
        match key.code {
            ...
            KeyCode::Enter => {
                ...
                self.seq += 1;
                agent.followup(crate::cli::user_message_with_text(format!("u-{}", self.seq), text));
                Ok(false)
            }
            KeyCode::Backspace => {
