...
use dsh_rs::api::services::{AgentRegistryService, AgentView, LlmService};
use dsh_rs::cli::{last_assistant_text, load_profile, parse_args, render_event, repair_crash_turns};
use dsh_rs::types::AgentOptions;
...
async fn cmd_run(
    ...
    agent.followup(dsh_rs::cli::user_message_with_text("u-1", prompt));
...
async fn cmd_chat(
    ...
    if interactive && !flags.contains_key("line") {
        ...
        dsh_rs::cli::tui::run_chat(&ctx, &agent).await?;
...
async fn line_chat(ctx: &Context, agent: &Arc<dyn AgentView>) -> Result<(), String> {
    ...
    for line in stdin.lock().lines() {
        ...
        seq += 1;
        agent.followup(dsh_rs::cli::user_message_with_text(format!("u-{seq}"), line));
        agent.when_idle().await;
        flush_session(ctx, agent).await;
        println!("{}", last_assistant_text(agent.session().as_ref()));
    }
...
async fn cmd_transcript(
    ...
    let ctx = Context::new();
    let config = dsh_rs::bundle::BaseConfig {
        store_dir: Some(std::path::PathBuf::from(&store_dir)),
        ..Default::default()
    };
    dsh_rs::bundle::install_base(&ctx, config).await?;
...
async fn cmd_dump_config(
    ...
    let manifest = ctx
        .get::<dsh_rs::api::services::ManifestService>(dsh_rs::api::MANIFEST_SERVICE)
    ...
    let missing = dsh_rs::api::manifest::ManifestRegistry::new().validate_coverage(&manifests);
    if missing.is_empty() {
...
async fn cmd_plugin_load(
    ...
    let plugin = dsh_rs::bundle::load_dynamic_plugin(std::path::Path::new(path))
    ...
    let tools = ctx
        .get::<dsh_rs::api::services::ToolsService>(dsh_rs::api::TOOLS_SERVICE)
...
async fn boot(
    flags: &std::collections::HashMap<String, String>,
) -> Result<(Context, Vec<cordis::FiberHandle>, dsh_rs::bundle::BaseConfig), String> {
    let ctx = Context::new();
    let (handles, config) = if let Some(profile) = flag(flags, "profile") {
        let profile = load_profile(&profile)?;
        let handles = dsh_rs::bundle::install_profile(&ctx, &profile).await?;
        let config = profile
            .get("config")
            .map(dsh_rs::bundle::BaseConfig::from_value)
            .unwrap_or_default();
        (handles, config)
    } else {
        ...
        let handles = dsh_rs::bundle::install_base(&ctx, config.clone()).await?;
        (handles, config)
    };
...
fn discover_default_config() -> dsh_rs::bundle::BaseConfig {
    ...
    for path in candidates {
        ...
        match serde_json::from_str::<Value>(&text) {
            Ok(value) => {
                ...
                return dsh_rs::bundle::BaseConfig::from_value(&value);
            }
            Err(err) => {
...
async fn create_agent(
    ...
    let agents = ctx
        .get::<AgentRegistryService>(dsh_rs::api::AGENTS_SERVICE)
