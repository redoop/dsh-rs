...
use dsh_rs::llm::{ContentBlock, MessageSource};
use dsh_rs::session::{
...
fn request_header_folds_latest_snapshot() {
    ...
    let header = dsh_rs::session::EpochHeader {
        config: dsh_rs::llm::LlmCallConfig {
            provider: "mock".into(),
            model: "mock-1".into(),
            ..Default::default()
        },
...
async fn jsonl_persistence_roundtrip_and_crash_repair() {
    let dir = dsh_rs::session::persistence::temp_dir("roundtrip");
...
async fn load_with_repair_helper() {
    let dir = dsh_rs::session::persistence::temp_dir("repair");
    let backend = Arc::new(JsonlPersistence::new(dir.clone()));
    let ctx = cordis::Context::new();
    let store = SessionStore::new(ctx);
    store.attach_persistence(backend.clone());
...
async fn jsonl_plugin_attaches_to_store() {
    let dir = dsh_rs::session::persistence::temp_dir("plugin");
    let ctx = cordis::Context::new();
    ...
    let handle = ctx
        .get::<Arc<dyn dsh_rs::api::services::SessionPersistenceApi>>("sessionPersistence")
