...
use cordis::Context;
use dsh_rs::api::services::{BoxFuture, LlmAdapterApi, LlmService};
use dsh_rs::types::{
...
pub async fn boot_scripted(ctx: &Context, install: bool, script: Vec<Vec<StreamChunk>>) {
    if install {
        dsh_rs::bundle::install_base_default(ctx).await.expect("base bundle boots");
    }
    let runtime = ctx
...
pub async fn create_agent(ctx: &Context) -> Arc<dyn dsh_rs::api::services::AgentView> {
    let agents = ctx
