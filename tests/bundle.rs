...
use cordis::Context;
use dsh_rs::api::services::ManifestService;
use dsh_rs::bundle::{BaseConfig, install_base, install_profile};
use dsh_rs::session::SessionPersistence;
use dsh_rs::types::{AgentOptions, SessionEventData};
use serde_json::json;
...
async fn base_bundle_boots_and_runs_an_agent() {
    ...
    assert!(ctx.get::<dsh_rs::api::services::LlmService>("llm").is_some());
    assert!(ctx.get::<dsh_rs::api::services::SessionService>("sessions").is_some());
    assert!(ctx.get::<dsh_rs::api::services::ToolsService>("tools").is_some());
    assert!(ctx.get::<dsh_rs::api::services::SystemPromptService>("systemPrompt").is_some());
    assert!(ctx.get::<dsh_rs::api::services::AgentRegistryService>("agents").is_some());
    ...
    let agent = agents
        ...
        .unwrap();
    agent.followup(dsh_rs::types::Message::user("u-1", vec![dsh_rs::types::ContentBlock::text("hello")]));
...
async fn manifest_registry_declares_full_coverage() {
    ...
    let missing =
        dsh_rs::api::manifest::ManifestRegistry::new().validate_coverage(&manifests);
...
async fn bundle_with_persistence_records_sessions() {
    ...
    let agent = agents
        ...
        .unwrap();
    agent.followup(dsh_rs::types::Message::user("u-1", vec![dsh_rs::types::ContentBlock::text("hi")]));
    agent.when_idle().await;

    let store = ctx.require::<dsh_rs::api::services::SessionService>("sessions").unwrap();
    store.flush(agent.id()).await.unwrap();
    assert!(dir.join("persisted-session.jsonl").exists());

    let backend = dsh_rs::session::JsonlPersistence::new(dir.clone());
