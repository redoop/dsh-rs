...
fn assembler_max_tokens_drops_tool_calls() {
    let mut a = BlockAssembler::new();
    let chunks = dsh_rs::llm::adapters::mock::MockAdapter::tool_call_response(
        "call-1",
        "bash",
...
fn assembler_interrupted_blocks_keep_text_only() {
    ...
    a.push(&dsh_rs::llm::adapters::mock::MockAdapter::tool_call_response(
        "call-9",
        "bash",
        serde_json::json!({}),
    )[0].clone());
...
async fn mock_adapter_echoes_last_user_message() {
    let adapter = dsh_rs::llm::adapters::mock::MockAdapter::new();
...
async fn runtime_routes_and_rejects_duplicates() {
    ...
    let adapter: Arc<dyn LlmAdapter> = Arc::new(dsh_rs::llm::adapters::mock::MockAdapter::new());
    runtime.register_adapter(&["mock"], adapter).unwrap();
    assert!(runtime.has_provider("mock"));
    assert!(runtime
        .register_adapter(&["mock"], Arc::new(dsh_rs::llm::adapters::mock::MockAdapter::new()))
        .is_err());
